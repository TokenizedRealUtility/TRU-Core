...
void DesktopWalletCore::assertPreparedOneInputV1(
    const std::string& unsignedTxHex,
    const std::string& expectedTxid,
    std::uint32_t expectedVout) const {
    const Tx tx=parsePreparedTx(unsignedTxHex);
    if(tx.version!=1 || tx.lockTime!=0 || tx.vin.size()!=1 ||
       tx.vin[0].txid!=expectedTxid || tx.vin[0].vout!=expectedVout ||
       tx.vin[0].sequence!=0xffffffffu || !tx.vin[0].scriptSig.empty())
        throw std::runtime_error("unexpected prepared transaction version/input/lockTime/sequence");
}

std::vector<tru_desktop_07::Output> DesktopWalletCore::inspectPreparedOutputs(
    const std::string& unsignedTxHex) const {
    const Tx tx=parsePreparedTx(unsignedTxHex);
    std::vector<tru_desktop_07::Output> result;
    for(const auto& v : tx.vout)
        result.push_back({v.amount,toHex(v.script)});
    return result;
}

std::string DesktopWalletCore::inspectPreparedMetadataTail(
    const std::string& unsignedTxHex) const {
    const Tx tx=parsePreparedTx(unsignedTxHex);
    return toHex(tx.metadataTail);
}

std::string DesktopWalletCore::inspectPreparedMetadataJson(
    const std::string& unsignedTxHex) const {
    const Tx tx=parsePreparedTx(unsignedTxHex);
    if(tx.metadataTail.empty() || tx.metadataTail[0]!=1)
        return {};
    std::size_t pos=1;
    const auto size=readVarInt(tx.metadataTail,pos);
    if(size>4*1024*1024ULL || pos+size!=tx.metadataTail.size())
        throw std::runtime_error("invalid prepared metadata envelope");
    return std::string(tx.metadataTail.begin()+static_cast<std::ptrdiff_t>(pos),
                       tx.metadataTail.end());
}
