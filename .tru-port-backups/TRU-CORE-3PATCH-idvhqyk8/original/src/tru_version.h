...
inline std::string parseCoreVersionFromUserAgent(const std::string& ua) {
    ...
    return validRelease(v) && v != "unreported" ? v : std::string("legacy/unreported");
}

inline bool isReportedVersion(const std::string& v) {
    return !v.empty() && v != "unreported" && v != "legacy/unreported";
}
